...
// Include authentication and database configuration files require_once '../config/auth.php'; require_once '../config/db.php'; 
 
// Start session if not already started if (session_status() == PHP_SESSION_NONE) {     session_start(); 
} 
 
// Redirect to login page if not authenticated redirectIfNotAuthenticated('login.php'); 
...
// Store borrowed books in an array while ($borrowed_books_stmt->fetch()) {     $borrowed_books[] = [ 
'id' => $borrow_id, 
'title' => $book_title, 
'author' => $book_author, 
...
$books = []; while ($row = $books_stmt->fetch_assoc()) { 
    $books[] = $row; 
} 
$books_stmt->close(); 
...
(is_null($borrowed_book['returned_at'])): ?> 
                                    ...
                                    <form method="POST" action="return_book.php"> 
                                        <input type="hidden" name="borrow_id" value="<?php echo $borrowed_book['id']; ?>">                                         <button type="submit" class="button">Return</button> 
                                    </form> 
                                <?php endif; ?> 
                            </td> 
                        </tr> 
